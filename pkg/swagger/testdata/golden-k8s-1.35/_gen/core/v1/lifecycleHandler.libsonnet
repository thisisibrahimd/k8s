{
  ...
  '#':: d.pkg(name='lifecycleHandler', url='', help='"LifecycleHandler defines a specific action that should be taken in a lifecycle hook. One and only one of the fields, except TCPSocket must be specified."'),
  ...
  exec: {
    ...
    mapCommand(f): { exec+: { command: std.map(f, super.command) } },
  ...
  httpGet: {
    ...
    mapHttpHeaders(f): { httpGet+: { httpHeaders: std.map(f, super.httpHeaders) } },
    ...
    mapHttpHeadersByName(name, transformFunc): { httpGet+: { httpHeaders: [if c.name == name then transformFunc(c) else c for c in super.httpHeaders] } },
  ...
  sleep: {
    ...
    withSeconds(seconds): { sleep+: { seconds: seconds } },
  },
