{
  ...
  '#configSource':: d.obj(help='"NodeConfigSource specifies a source of node configuration. Exactly one subfield (excluding metadata) must be non-nil. This API is deprecated since 1.22"'),
  configSource: {
  ...
  '#mapPodCIDRs':: d.fn(help='"podCIDRs represents the IP ranges assigned to the node for usage by Pods on that node. If this field is specified, the 0th entry must match the podCIDR field. It may contain at most 1 value for each of IPv4 and IPv6."\n\n**Note:** This function maps each element using the provided function', args=[d.arg(name='f', type=d.T['function'])]),
  mapPodCIDRs(f): { podCIDRs: std.map(f, super.podCIDRs) },
  '#mapTaints':: d.fn(help="\"If specified, the node's taints.\"\n\n**Note:** This function maps each element using the provided function", args=[d.arg(name='f', type=d.T['function'])]),
  mapTaints(f): { taints: std.map(f, super.taints) },
