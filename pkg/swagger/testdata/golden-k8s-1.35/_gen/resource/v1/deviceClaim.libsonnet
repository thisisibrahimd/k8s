{
  ...
  mapConstraints(f): { constraints: std.map(f, super.constraints) },
  ...
  mapRequests(f): { requests: std.map(f, super.requests) },
  ...
  mapRequestsByName(name, transformFunc): { requests: [if c.name == name then transformFunc(c) else c for c in super.requests] },
