{
  ...
  '#mapActive':: d.fn(help='"A list of pointers to currently running jobs."\n\n**Note:** This function maps each element using the provided function', args=[d.arg(name='f', type=d.T['function'])]),
  mapActive(f): { active: std.map(f, super.active) },
  '#mapActiveByName':: d.fn(help='"A list of pointers to currently running jobs."\n\n**Note:** This function maps the element matching name using the provided function', args=[d.arg(name='name', type=d.T.string), d.arg(name='transformFunc', type=d.T['function'])]),
  mapActiveByName(name, transformFunc): { active: [if c.name == name then transformFunc(c) else c for c in super.active] },
